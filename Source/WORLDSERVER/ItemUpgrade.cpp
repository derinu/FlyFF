...
void CItemUpgrade::BarunaWakeUp( CUser* pUser, CItemElem* pWarpon )
{    
    int nGold = 100000;
    if( IsValidObj( pUser ) )
    {
        ...
        if( pUser->m_vtInfo.VendorIsVendor() )
            return;
        if( pUser->m_Inventory.IsEquip( pWarpon->m_dwObjId ) )
            return;
        ...
        if( pUser->GetGold() >= nGold )
        {
            ...
            pUser->AddPlaySound( SND_INF_UPGRADESUCCESS );
		    g_UserMng.AddCreateSfxObj((CMover *)pUser, XI_INT_SUCCESS, pUser->GetPos().x, pUser->GetPos().y, pUser->GetPos().z);
            pUser->AddDefinedText( TID_MMI_BARUNA_WAKEUP_TEXT01, "\"%s\"", pWarpon->GetName() );
        }
        else
        {
        pUser->AddDefinedText( TID_MMI_BARUNA_WAKEUP_E_TEXT05 );
        return;
        }
    }
}
void CItemUpgrade::BarunaWakeUpCancel( CUser* pUser, CItemElem* pWarpon, CItemElem* pPotion )
{
    if( IsValidObj( pUser ) )
    {
        ...
        if( pUser->m_Inventory.IsEquip( pWarpon->m_dwObjId ) )
            return;
        if( pWarpon->GetPiercingSize() >= 1)
            return;
        if( pPotion->GetProp()->dwID != II_SYS_SYS_SCR_BARUNAOBLIVION )
            return;

            pUser->UpdateItemEx( (BYTE)( pWarpon->m_dwObjId ), UI_RANDOMOPTITEMID, pWarpon->GetRandomOptItemId() );
            pUser->AddPlaySound( SND_INF_UPGRADESUCCESS ); 
		    g_UserMng.AddCreateSfxObj((CMover *)pUser, XI_INT_SUCCESS, pUser->GetPos().x, pUser->GetPos().y, pUser->GetPos().z);
            pUser->AddDefinedText( TID_MMI_BARUNA_WAKEUPCANCEL_TEXT01, "\"%s\"" , pWarpon->GetName() );
