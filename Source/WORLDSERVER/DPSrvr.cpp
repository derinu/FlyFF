...
void CDPSrvr::OnBaruna( CAr & ar, DPID dpidCache, DPID dpidUser, LPBYTE lpBuf, u_long uBufSize)
{
	...
	if( IsValidObj( pUser ) )
	{
        ...
        case BARUNA_WAKE_UP:
	            ...
	            {
	            pUser->AddGold(-100000,FALSE);
	            b = DoUseItemTarget_GenRandomOption( pUser, pItemElem0, CRandomOptionProperty::eAwakening, 0, TID_GAME_INVALID_TARGET_ITEM, TID_GAME_AWAKE_OR_BLESSEDNESS01, "r", "::Awake" );
	            if( b )
	            {
        ...
        case BARUNA_WAKE_UP_CANCEL:
	           {
	            ...
	            if( IsUsableItem( pItemElem0 ) == FALSE || IsUsableItem( pItemElem1 ) == FALSE )
	               return;
				b	= DoUseItemTarget_InitializeRandomOption( pUser, pItemElem0, CRandomOptionProperty::eAwakening,
					TID_GAME_AWAKECANCEL_INFO, TID_GAME_AWAKECANCEL,
					"r", "::AwakeCancel" );  
	            if( b )
	            {
