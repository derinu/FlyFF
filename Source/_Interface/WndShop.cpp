...
void CWndConfirmBuy::OnOK()
{
	...
	if( m_bCart )
	{
		if( pWndShop )
		{
			pWndShop->AddCartItem( (DWORD)nBuy, m_pItemElem, cTab );
		}
		return;
	}
#endif //__SHOPPING_CART
...
{ 
	...
	m_dwTotalPenya = 0;
	ZeroMemory( &pCartItem, sizeof( CART_ITEM ) );
#endif //__SHOPPING_CART
	m_pWndWarning	= NULL;
	m_bSexSort    = FALSE;
	m_bLevelSort  = FALSE;
...
#ifdef __SHOPPING_CART
void CWndShop::OnLButtonDblClk( UINT nFlags, CPoint point )
{
		LPWNDCTRL pWndCtrl = GetWndCtrl( WIDC_STATIC3 );
		int cSlot = 0;
		if( point.y - pWndCtrl->rect.top > 32 )
			cSlot += 7;

		cSlot += (int)( ( point.x - pWndCtrl->rect.left ) / 37 );
		DeleteCartItem( cSlot );
}
BOOL CWndShop::OnDropIcon(  LPSHORTCUT pShortcut, CPoint point )
{
	if( pShortcut->m_dwShortcut == SHORTCUT_ITEM  )
		GetParentWnd()->OnChildNotify( WIN_ITEMDROP, m_nIdWnd, (LRESULT*)pShortcut ); 
	return TRUE;
...
{ 
		...
		if( nID == 10 || nID == 11 || nID == 12 || nID == 13) // item
		{
			...
			if( pFocusItem )
			{
				...
				if( pProp )
				{
					...
					m_pWndConfirmBuy->m_pItemElem = (CItemElem*)pFocusItem;
					m_pWndConfirmBuy->m_bCart = TRUE;
	...
	case WIDC_BT_BUY:
		{
			g_DPlay.SendBuyItemCart( pCartItem );
		}
	case WIDC_BT_CANCEL:
		{
			Destroy();
...
#ifdef __SHOPPING_CART
void CWndShop::ClearCart()
{
	for( int i = 0; i < MAX_CART_ITEM; i++ )
	{
		pCartItem.pItemElem[i] = NULL;
		pCartItem.dwObjID[i] = NULL;
		pCartItem.cTab[i] = NULL;
		pCartItem.dwItemID[i] = NULL;
		pCartItem.dwNum[i] = NULL;
	}
}
...
#endif//__SHOPPING_CART_FIX
/*****************************************************************/
		DWORD dwTotalCost = pItemElem->GetCost() * dwNum;
		DWORD dwTotalCart = 0;
		...
		for( int i = 0;i < MAX_CART_ITEM; i++ )
		{
			if( pCartItem.pItemElem[i] )
			{
				dwTotalCost += pCartItem.dwNum[i] * pCartItem.pItemElem[i]->GetCost();
				dwTotalCart++;
			}
		}
		...
		for( int i = 0; i < MAX_CART_ITEM; i++ )
		{
			if( pCartItem.pItemElem[i] && pCartItem.pItemElem[i]->GetProp()->dwID == pItemElem->GetProp()->dwID && pCartItem.cTab[i] == cTab )
			{
				if( pCartItem.pItemElem[i]->GetProp()->dwPackMax > pCartItem.dwNum[i] )
				{
					if( pCartItem.dwNum[i] + dwNum > pCartItem.pItemElem[i]->GetProp()->dwPackMax )
					{
						DWORD dwRemain = ( pCartItem.dwNum[i] + dwNum ) - pCartItem.pItemElem[i]->GetProp()->dwPackMax;
						pCartItem.dwNum[i] = pCartItem.pItemElem[i]->GetProp()->dwPackMax;
						AddCartItem( dwRemain, pItemElem, cTab );
						return;
					}
					else
					{
						pCartItem.dwNum[i] += dwNum;
						UpdateCart();
						return;
					}
				}
		...
		for( int i = 0; i < MAX_CART_ITEM; i++ )
		{
			if( !pCartItem.pItemElem[i] )
			{
				if( pItemElem->GetProp()->dwPackMax >= dwNum )
				{

					pCartItem.pItemElem[i] = pItemElem;
					pCartItem.dwObjID[i] = pItemElem->m_dwObjId;
					pCartItem.cTab[i] = cTab;
					pCartItem.dwItemID[i] = pItemElem->GetProp()->dwID;
					pCartItem.dwNum[i] = dwNum;
				...
				else
				{
					...
					pCartItem.cTab[i] = cTab;
					pCartItem.dwItemID[i] = pItemElem->GetProp()->dwID;
					pCartItem.dwNum[i] = pItemElem->GetProp()->dwPackMax;

					AddCartItem( dwNum - pItemElem->GetProp()->dwPackMax, pItemElem, cTab );
					return;
				}	
				break;
			}
...
void CWndShop::DeleteCartItem( CHAR cSlot )
{
	pCartItem.pItemElem[cSlot] = NULL;
	pCartItem.dwNum[cSlot] = NULL;
	UpdateCart();
}
void CWndShop::UpdateCart()
{
	...
	string.Format( _T( "%d" ), dwTotalCost );
	pCost->SetTitle( string );

	CWndStatic* pCart = (CWndStatic*) GetDlgItem( WIDC_STATIC_CARTCNT );
	string.Format( _T( "( %d / 12 )"), dwTotalCart );
	pCart->SetTitle( string );

	if( dwTotalCart >= MAX_CART_ITEM )
		pCart->m_dwColor = 0xFFFF0033;
	else
