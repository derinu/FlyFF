...
{ 
	...
	if( pWndGroupBox )
	{
		pWndGroupBox->m_dwColor = D3DCOLOR_ARGB( 255, 238, 221, 130 );
	...
	if( lpCharacter )
	{
		...
		m_texChar.LoadTexture( g_Neuz.m_pd3dDevice, MakePath( "Data\\",lpCharacter->m_szChar ), 0xffff00ff, TRUE );
...
void CWndDialog::MakeAnswerButton()
{
	...
	if( m_nCurArray == m_strArray.GetSize() - 1 )
	{
		...
		for( i = 0; i < m_nWordButtonNum; i++ )
		{
            ...
            CRect meurect;
			meurect = CRect( x, lpWndCtrl->rect.top - 20, x + 70, lpWndCtrl->rect.bottom + 10 );
			...
			{  			
				...
				m_apWndAnswer[ j ]->Create( "Yes", WBS_CHILD, rect, this, nWndId );
			...
			if( strcmp( pWordButton->szWord, "__NO__" ) == 0 )
			{
				...
				strTexture = "ButtNormal00.tga";
				m_apWndAnswer[ j ] = new CWndAnswer;
				m_apWndAnswer[ j ]->Create( "No", WBS_CHILD, rect, this, nWndId );
				m_apWndAnswer[ j ]->SetTexture( D3DDEVICE, MakePath( DIR_THEME, strTexture ), 1 );
				m_apWndAnswer[ j ]->FitTextureSize();
				m_apWndAnswer[ j ]->m_pWordButton = (WORDBUTTON*)	pWordButton;
				j++;
			...
			if( strcmp( pWordButton->szWord, "__CANCEL__" ) == 0 )
			{
				...
				strTexture = "ButtNormal00.tga";
				m_apWndAnswer[ j ] = new CWndAnswer;
				m_apWndAnswer[ j ]->Create( "Cancel", WBS_CHILD, rect, this, nWndId );
				m_apWndAnswer[ j ]->SetTexture( D3DDEVICE, MakePath( DIR_THEME, strTexture ), 1 );
				m_apWndAnswer[ j ]->FitTextureSize();
				m_apWndAnswer[ j ]->m_pWordButton = (WORDBUTTON*)	pWordButton;
				j++;
			...
			if( strcmp( pWordButton->szWord, "__OK__" ) == 0 )
			{
				...
				strTexture = "ButtNormal00.tga";
				m_apWndAnswer[ j ] = new CWndAnswer;
				m_apWndAnswer[ j ]->Create( "Ok", WBS_CHILD, rect, this, nWndId );
				m_apWndAnswer[ j ]->SetTexture( D3DDEVICE, MakePath( DIR_THEME, strTexture ), 1 );
				m_apWndAnswer[ j ]->FitTextureSize();
				m_apWndAnswer[ j ]->m_pWordButton = (WORDBUTTON*)	pWordButton;
				j++;
