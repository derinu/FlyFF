				...
				if( lpShortcut->m_dwShortcut == SHORTCUT_ITEM )
				{
					...
					if( pItemElem  )
					{
						if( pItemElem->GetProp()->dwPackMax > 1 )
						{
							...
							p2DRender->TextOut( point.x + 32 - size.cx, point.y + 32 - size.cy, szTemp, 0xff00220b );
							p2DRender->TextOut( point.x + 31 - size.cx, point.y + 31 - size.cy, szTemp, 0xff00ffff );
						}
					}
					else
	...
	if( pShortcut->m_dwShortcut == SHORTCUT_APPLET )
	{
		...
		//	{
			...
			else
			{
				...
				strEdit.AddString( string, 0xff00ffff );
			}
			//CEditString strEdit;
...
//	return CWndBase::Create( 0 | /*WBS_MODAL|*/ WBS_MANAGER | WBS_SOUND | WBS_NOFRAME, rect, &g_WndMng, APP_TASKBAR );
	...
	else if( g_Option.m_nResWidth == 1680 || g_Option.m_nResWidth == 1920 )
	{
		m_nMaxSlotApplet = MAX_SLOT_APPLET;
