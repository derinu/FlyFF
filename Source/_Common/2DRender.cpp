...
BOOL CTexture::LoadTexture( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR pFileName, D3DCOLOR d3dKeyColor, BOOL bMyLoader )
{
	...
	if( bMyLoader == FALSE ) // dds only
	{
		...
		if( hr != S_OK )
		{
			LPCTSTR szErr = Error( "hr != S_OK CTexture::LoadTexture : %s read error", pFileName );
			ADDERRORMSG( szErr );
			return FALSE;		
		}
	...
	if( strstr( szFileName, ".bmp" ) )
	{
		...
		//#define BMP32BIT
			...
			Error( "LoadBMP == false CTexture::LoadTexture : %s read error", pFileName );
	}
	else // bmp or tga
	if( strstr( szFileName, ".tga" ) )
	{
		...
		#define BMP32BIT
			...
			Error( "LoadTGA == false CTexture::LoadTexture : %s read error", pFileName );
